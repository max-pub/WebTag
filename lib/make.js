...
export default class WebTag {
	...
	scriptParts = {
		...
		onDataChange: /^\s*(async)?\s*\$onDataChange\s*\(/m,
		onInputChange: /^\s*(async)?\s*\$onInputChange\s*\(/m,
		onLoad: /^\s*(async)?\s*\$onLoad\s*\(/m,
		onReady: /^\s*(async)?\s*\$onReady\s*\(/m,
		event: /\s*this\.\$event\s*\(/m,
		QuerySelector: /[a-zA-Z]+.Q\s*\(/m,
		XPATH: /[a-zA-Z]+.X\s*\(/m,
		ATTR: /[a-zA-Z]+.A[^a-zA-Z0-9]/m,
	...
	adaptScript() {
		// this.output = this.output.replace('___TAGNAME___', this.tagName);
	...
	replace(key, value) {
		// console.log('match', new RegExp(`___${key}___`, 'g'), this.output.match(new RegExp(`___${key}___`, 'g')))
		// console.log('replace', new RegExp(`___${key}___`, 'g'), value)
		this.output = this.output.replace(new RegExp(`___${key}___`, 'g'), value);
	}
